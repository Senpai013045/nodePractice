 ...
 * @method
 * @property {function} get function that you pass string on to get the value
 */
/**
 * dataReceived from the router
 * @typedef {Object} dataReceived
 * @property {{tokenid:string}} headers headers of the request
 * @property {string} trimmedPath path but without slashes on front or back
 * @property {query} searchParams search params
 * @property {"get"|"post"} method  get or post
        ...
        _data.read("users", phoneNumber, (err, userData) => {
          if (!err && userData) {
            _data.delete("users", phoneNumber, (error) => {
              if (!error) {
                //delete each checks associated with this data
                const checks =
                    ...
                    if (!userReadError && userData) {
                      ...
                      if (checks.length < config.maxChecks) {
                        ...
                        if (checkId) {
                          ...
                          callbackStatusCodeAndData(500, {
                            message:
                              "An error occurred while creating the checkId",
                          });
                        }
                      } else {
                        callbackStatusCodeAndData(400, {
 ...
 * @param {function} callbackStatusCodeAndData function that injects statuscode and data to be sent back (object)
 */
//check id is requires, rest are optional
handlers._checks.put = function (dataReceived, callbackStatusCodeAndData) {
  //check for id via payload
  const checkId =
    typeof dataReceived.payload.checkId === "string" &&
    dataReceived.payload.checkId.trim().length === 20
      ? dataReceived.payload.checkId.trim()
      : false;
  if (checkId) {
    //optional data
    ...
    //check if at least one of them is present
    if (protocol || url || method || successCodes || timeoutSeconds) {
      //lookup the check
      _data.read("checks", checkId, (errorCheckRead, checkData) => {
        if (!errorCheckRead && checkData) {
          //get token from headers
          const tokenId =
            typeof dataReceived.headers.tokenid === "string"
              ? dataReceived.headers.tokenid
          ...
          if (tokenId) {
            handlers._tokens.verifyToken(
              ...
              (isTokenValid) => {
                if (isTokenValid) {
                  ...
                  _data.update("checks", checkId, checkData, (updateError) => {
                    if (!updateError) {
                      callbackStatusCodeAndData(200, checkData);
                    } else {
                      callbackStatusCodeAndData(500, {
            ...
            callbackStatusCodeAndData(403, {
              message: "No tokenid found in headers",
            });
          }
        } else {
          callbackStatusCodeAndData(400, {
    ...
    callbackStatusCodeAndData(400, { message: "Missing required checkId" });
  }
};
/**
 *delete handler for checks
 * @param {dataReceived} dataReceived object that goes into this handler
 * @param {function} callbackStatusCodeAndData function that injects statuscode and data to be sent back (object)
 */
handlers._checks.delete = function (dataReceived, callbackStatusCodeAndData) {
  //get checkId from searchParams
  const checkId =
    typeof dataReceived.searchParams.get("checkId") === "string" &&
    dataReceived.searchParams.get("checkId").trim().length === 20
      ? dataReceived.searchParams.get("checkId").trim()
      : false;

  if (checkId) {
    //lookup checkId
    _data.read("checks", checkId, (readError, checkData) => {
            ...
            (isValid) => {
              if (isValid) {
                ...
                _data.delete("checks", checkId, (deleteError) => {
                  if (!deleteError) {
                    //lookup user and get all the checks
                    _data.read(
                      "users",
                      checkData.phoneNumber,
                      (readUserError, userData) => {
                        if (!readUserError && userData) {
                          ...
                          if (checkPosition > -1) {
                            ...
                            callbackStatusCodeAndData(500, {
                              message:
                                "Could not find the specified check on user data, removal of the id from user data failed",
                            });
                          }
                        } else {
                          callbackStatusCodeAndData(500, {
                            message:
                              "Could not find the user associated with the check, check id removal from user data failed",
                          });
                        }
                      }
                    );
                    ...
                    callbackStatusCodeAndData(500, {
                      message: "Could not delete check data",
                    });
                  }
                });
              } else {
    ...
    callbackStatusCodeAndData(400, {
      message: "No checkId found in the query",
    });
  }
};
